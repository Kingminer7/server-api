...
public:
    static Result<std::shared_ptr<PopupSetting>> parse(std::string const& key, std::string const& modID, matjson::Value const& json) {
...
protected:
    ...
    bool init(std::shared_ptr<PopupSetting> setting, float width) {
        ...
        m_buttonSprite = ButtonSprite::create("Debug", "goldFont.fnt", "GJ_button_04.png", .8f);
...
public:
    static PopupSettingNode* create(std::shared_ptr<PopupSetting> setting, float width) {
...
SettingNodeV3* PopupSetting::createNode(float width) {
    return PopupSettingNode::create(
        std::static_pointer_cast<PopupSetting>(shared_from_this()),
        width
    );
}

$execute {
    (void)Mod::get()->registerCustomSettingType("popup-setting", &PopupSetting::parse);
}